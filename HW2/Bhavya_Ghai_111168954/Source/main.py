...
def Cylindrical_warping(img1, img2, img3):
	...
	# Write your codes here
    	...
    	f = 700
    	K = np.array([[f, 0, w/2], [0, f, h/2], [0, 0, 1]]) # mock calibration matrix
    	img1cyl = cylindricalWarpImage(img1, K)

	h,w = img2.shape
        f = 700
        K = np.array([[f, 0, w/2], [0, f, h/2], [0, 0, 1]]) # mock calibration matrix
        img2cyl = cylindricalWarpImage(img2, K)

	h,w = img3.shape
        f = 700
        K = np.array([[f, 0, w/2], [0, f, h/2], [0, 0, 1]]) # mock calibration matrix
        img3cyl = cylindricalWarpImage(img3, K)

	# first, make some room around img1
        img1 = cv2.copyMakeBorder(img1cyl[0],50,50,300,300, cv2.BORDER_CONSTANT)
	mask1 = cv2.copyMakeBorder(img1cyl[1],50,50,300,300, cv2.BORDER_CONSTANT)
	
	img2 = cv2.copyMakeBorder(img2cyl[0],50,50,300,300, cv2.BORDER_CONSTANT)
	mask2 = cv2.copyMakeBorder(img1cyl[1],50,50,300,300, cv2.BORDER_CONSTANT)
	img3 = cv2.copyMakeBorder(img3cyl[0],50,50,300,300, cv2.BORDER_CONSTANT)
	mask3 = cv2.copyMakeBorder(img1cyl[1],50,50,300,300, cv2.BORDER_CONSTANT)
	
	#Calculate the geometric transform (Affine) between img1 and img2
	...
	mask_affine2 = cv2.warpAffine(mask2, M, (img1.shape[1],img1.shape[0]))
	
	dst1 = img1.copy()
	dst1[mask_affine2 == 255] = out1[mask_affine2 == 255]
	
	#Calculate the geometric transform (Affine) between out1 and img3
        (M, pts1, pts2, mask) = getTransform(img3, out1, method='affine')
        out2 = cv2.warpAffine(img3, M, (out1.shape[1],out1.shape[0]))
	mask_affine3 = cv2.warpAffine(mask3, M, (img1.shape[1],img1.shape[0]))
        dst2 = dst1.copy()
        dst2[mask_affine3 == 255] = out2[mask_affine3 == 255]
	
	output_image = dst2 	     
	#print("Error: ",RMSD(2,output_image,))
